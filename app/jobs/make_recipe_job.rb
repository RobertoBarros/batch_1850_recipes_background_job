class MakeRecipeJob < ApplicationJob
  queue_as :default
  ...
  def perform(recipe)
    ...
    response = client.chat(parameters: {
      ...
      messages: [{
        role: "user",
        content: "Give me a simple recipe for #{recipe.name} with the ingredients #{recipe.ingredients}.
                  Give me only the text of the recipe, without any of your own answer
                  like 'Here is a simple recipe'."}]
    })

    recipe.content = response["choices"][0]["message"]["content"]
    ...
    Turbo::StreamsChannel.broadcast_update_to('recipe-stream',
      target: recipe,
